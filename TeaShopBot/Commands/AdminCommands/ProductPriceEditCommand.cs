...
{
    public class ProductPriceEditCommand : TelegramCommand
    {
        public override string Name => "Цена";
        ...
        public override async Task Execute(Update update, ITelegramBotClient client, CancellationToken cancellationToken)
        {
            ...
            var probablyProductId = text.Substring(4, index - 4);
            ...
            try
            {
                if (long.TryParse(probablyProductId, out productId))
                {
                    ...
                    if (product != null)
                    {
                        product.ProductPrice = Convert.ToDecimal(text.Substring(index + 1));
                        await UpdateProduct(product);

                        await client.SendTextMessageAsync(
                            chatId: chatId,
                            text: $"💪🏾 Цена товара изменена!\n\n" +

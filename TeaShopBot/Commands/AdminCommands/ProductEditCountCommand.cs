...
{
    public class ProductEditCountCommand : TelegramCommand
    {
        public override string Name => "Количество";
        ...
        public override async Task Execute(Update update, ITelegramBotClient client, CancellationToken cancellationToken)
        {
            ...
            var probablyProductId = text.Substring(10, index - 10);
            ...
            try
            {
                if (long.TryParse(probablyProductId, out productId))
                {
                    ...
                    if (product != null)
                    {
                        product.ProductCount = Convert.ToInt32(text.Substring(index + 1));

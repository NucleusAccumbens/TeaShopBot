...
public class HerbRegionCallbackCommand : BaseCallbackCommand
{
    ...
    public override async Task CallbackExecute(Update update, ITelegramBotClient client)
    {
        if (update.CallbackQuery != null && update.CallbackQuery.Message != null)
        {
            ...
            try
            {
                ...
                {
                    await EditMenuMessage(update, client, HerbRegion.Altai);

                    return;
                }
                if (update.CallbackQuery.Data == "ICaucasus")
                {
                    await EditMenuMessage(update, client, HerbRegion.Caucasus);

                    return;
                }
                if (update.CallbackQuery.Data == "IKarelia")
                {
                    await EditMenuMessage(update, client, HerbRegion.Karelia);

                    return;
                }
                if (update.CallbackQuery.Data == "ISiberia")
                {
                    await EditMenuMessage(update, client, HerbRegion.Siberia);
                }
            }
    ...
    private void SetHerbRegionAndCommandState(HerbRegion herbsRegion, Update update)
    {

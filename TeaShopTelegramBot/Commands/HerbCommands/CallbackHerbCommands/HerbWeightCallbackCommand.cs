...
public class HerbWeightCallbackCommand : BaseCallbackCommand
{
    ...
    public override async Task CallbackExecute(Update update, ITelegramBotClient client)
    {
        if (update.CallbackQuery != null && update.CallbackQuery.Message != null)
        {
            ...
            try
            {
                ...
                {
                    await EditMenuMessage(update, client, HerbWeight.Fifty);

                    return;
                }
                if (update.CallbackQuery.Data == "F100")
                {
                    await EditMenuMessage(update, client, HerbWeight.OneHundred);

                    return;
                }
                if (update.CallbackQuery.Data == "F150")
                {
                    await EditMenuMessage(update, client, HerbWeight.OneHundredFifty);

                    return;
                }
                if (update.CallbackQuery.Data == "F200")
                {
                    await EditMenuMessage(update, client, HerbWeight.TwoHundred);

                    return;
                }
                if (update.CallbackQuery.Data == "F250")
                {
                    await EditMenuMessage(update, client, HerbWeight.TwoHundredFifty);
                }
            }
    ...
    private void SetHerbRegionAndCommandState(HerbWeight weight, HerbDto herbDto, Update update)
    {
